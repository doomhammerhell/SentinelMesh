...
async fn prove_batch(batch_path: PathBuf, output: Option<PathBuf>) -> Result<()> {
    ...
    let batch: sentinelmesh_core::ProbeBatch =
        serde_json::from_str(&batch_json).context("Failed to parse batch JSON")?;

    println!(
        "  {} Loaded batch with {} endpoints",
        "→".dimmed(),
...
async fn verify_proof(proof_path: PathBuf) -> Result<()> {
    ...
    if verified {
        ...
        println!(
            "  Batch Hash: {}",
            proof["batch_hash"].as_str().unwrap_or("unknown").dimmed()
        );
        println!(
            "  Sentinel: {}",
            proof["sentinel_id"].as_str().unwrap_or("unknown").dimmed()
        );
        println!(
            "  Endpoints: {}",
            proof["endpoint_count"]
                .as_u64()
                .unwrap_or(0)
                .to_string()
                .dimmed()
...
async fn show_status() -> Result<()> {
    ...
    println!(
        ...
        if cfg!(debug_assertions) {
            "Development (fast proofs)".yellow()
        } else {
            "Production (secure proofs)".green()
        }
    );
